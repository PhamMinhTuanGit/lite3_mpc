...
{
    if (abs(_stateEstimator.getResult().rpy(0)) >= 0.5 || abs(_stateEstimator.getResult().rpy(1)) >= 0.5)
    {
 ...
 */
template <typename T>
bool SafetyChecker<T>::checkPDesFoot(Quadruped<float> &_quadruped, LegController<float> &_legController)
{
    ...
    for (int leg = 0; leg < 4; leg++)
    {
        ...
        if (_legController.commands[leg].pDes(0) > maxPDes)
        {
            ...
            std::cout << "   commanded: " << _legController.commands[leg].pDes(0) << " | modified: " << maxPDes
                      << std::endl;
        ...
        if (_legController.commands[leg].pDes(1) > maxPDes)
        {
            ...
            std::cout << "   commanded: " << _legController.commands[leg].pDes(1) << " | modified: " << maxPDes
                      << std::endl;
        ...
        if (_legController.commands[leg].pDes(2) < -_quadruped._maxLegLength)
        {
            std::cout << "[CONTROL FSM] Safety: PDes leg: " << leg << " | coordinate: " << 2 << "\n";
            std::cout << "   commanded: " << _legController.commands[leg].pDes(2)
                      << " | modified: " << -_quadruped._maxLegLength << std::endl;
            _legController.commands[leg].pDes(2) = -_quadruped._maxLegLength;
            safePDesFoot = false;
        }
    }
 ...
 * Joint limits from Lite3 URDF (FL_HipX / FL_HipY / FL_Knee joints)
 */
template <typename T>
bool SafetyChecker<T>::checkJointLimit(LegController<float> &_legController)
{
    ...
    {
        // q(0): abad (HipX)
        if (_legController.datas[leg].q(0) < min_ab_ad_angle)
        {
            std::cout << "[SAFETY] Leg " << leg << " ab/ad (q0) = " << _legController.datas[leg].q(0)
                      << " | clamped to min " << min_ab_ad_angle << std::endl;
            _legController.datas[leg].q(0) = min_ab_ad_angle;
            safeJoint = false;
        }
        if (_legController.datas[leg].q(0) > max_ab_ad_angle)
        {
            std::cout << "[SAFETY] Leg " << leg << " ab/ad (q0) = " << _legController.datas[leg].q(0)
                      << " | clamped to max " << max_ab_ad_angle << std::endl;
        ...
        {
            std::cout << "[SAFETY] Leg " << leg << " hip (q1) = " << _legController.datas[leg].q(1)
                      << " | clamped to min " << min_hip_angle << std::endl;
        ...
        {
            std::cout << "[SAFETY] Leg " << leg << " hip (q1) = " << _legController.datas[leg].q(1)
                      << " | clamped to max " << max_hip_angle << std::endl;
        ...
        {
            std::cout << "[SAFETY] Leg " << leg << " knee (q2) = " << _legController.datas[leg].q(2)
                      << " | clamped to min " << min_knee_angle << std::endl;
        ...
        {
            std::cout << "[SAFETY] Leg " << leg << " knee (q2) = " << _legController.datas[leg].q(2)
                      << " | clamped to max " << max_knee_angle << std::endl;
...
bool SafetyChecker<T>::checkForceFeedForward(LegController<float> &_legController)
{
    ...
    for (int leg = 0; leg < 4; leg++)
    {
        ...
        if (_legController.commands[leg].forceFeedForward(0) > maxLateralForce)
        {
            std::cout << "[CONTROL FSM] Safety: Force leg: " << leg << " | coordinate: " << 0 << "\n";
            std::cout << "   commanded: " << _legController.commands[leg].forceFeedForward(0)
        ...
        if (_legController.commands[leg].forceFeedForward(1) > maxLateralForce)
        {
            std::cout << "[CONTROL FSM] Safety: Force leg: " << leg << " | coordinate: " << 1 << "\n";
            std::cout << "   commanded: " << _legController.commands[leg].forceFeedForward(1)
                      << " | modified: " << maxLateralForce << std::endl;
        ...
        if (_legController.commands[leg].forceFeedForward(2) > maxVerticalForce)
        {
            std::cout << "[CONTROL FSM] Safety: Force leg: " << leg << " | coordinate: " << 2 << "\n";
            std::cout << "   commanded: " << _legController.commands[leg].forceFeedForward(2)
                      << " | modified: " << -maxVerticalForce << std::endl;
            _legController.commands[leg].forceFeedForward(2) = maxVerticalForce;
            safeForceFeedForward = false;
        }
