...
private:
    ...
    std::shared_ptr<StateBase> cmpc_controller_;
    ...
    std::shared_ptr<StateBase> GetNextStatePtr(StateName state_name){
        switch(state_name){
            ...
            case StateName::kCMPC:{
                return cmpc_controller_;
            }
...
public:
    StateMachine(RobotType robot_type){
        ...
        cmpc_controller_ = std::make_shared<CMPCState>(robot_type, "cmpc", data_ptr);
