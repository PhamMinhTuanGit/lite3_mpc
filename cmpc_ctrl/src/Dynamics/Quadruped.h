...
class Quadruped {
 public:
  ...
  static T getSideSign(int leg) {
    // Vấn đề 1: Thứ tự chân (không thể sửa bằng getSideSign)
    // Vấn đề 2: Trục abad (-X vs +X) (không thể sửa bằng getSideSign)
    // - KHÔNG THỂ sửa lỗi trục abad (sửa lỗi cho thứ tự trong file urdf)
    const T sideSigns[4] = {-1, 1, -1, 1};
    //                      FR  FL  HR  HL // cheetah3
    // const T sideSigns[4] = {1, -1, 1, -1};
    // //                      FL  FR  HL  HR
    assert(leg >= 0 && leg < 4);
